...
# type("MineFilter", (MineBaseFilter,), {"MINE_FILED": 'company_id'})
...
class TranModeSerializer(serializers.ModelSerializer):
    ...
    tran_type_txt = serializers.CharField(source="get_tran_type_display")
    pay_status_txt = serializers.CharField(source="get_pay_status_display")
    pay_status_cls = serializers.SerializerMethodField()

    class Meta:
        model = models.TransactionRecord
        fields = "__all__"

    def get_pay_status_cls(self, obj):
        return models.TransactionRecord.pay_status_cls_map[obj.pay_status]


class TranView(ListPageNumberModelMixin, GenericViewSet):
    authentication_classes = [JwtAuthentication, JwtParamAuthentication, DenyAuthentication]

    # filter_backends = [type("MineFilter", (MineBaseFilter,), {"MINE_FILED": 'company_id'}), ]
    filter_backends = [MineFilter, TranSearchFilter]
